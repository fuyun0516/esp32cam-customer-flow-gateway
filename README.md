...
- 俯视安装时按“画面上方 → 下方 → 从底部离开”的顺序估算进店客流
- 当前日期客流计数和按小时柱状图
...
- 摄像头应固定在门框或天花板并接近垂直向下拍摄；画面上方必须对应店外，画面下方对应店内。
- 固件只有在运动目标依次经过上方区域、下方区域，并从画面底部离开后才估算一次进店客流；反向经过不计数。
- 它不能确认目标一定是人，也不能稳定区分并排行走或紧贴通过的多人。
